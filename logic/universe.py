...
class Universe:
    def __init__(self, entity_count=20):
        ...
        self.masses = (np.sqrt(np.arange(entity_count)*-1+entity_count) - 0.8) * 100
        self.masses[0] = 0
        self.randomize_pos()
        self.randomize_vel()
    ...
    def apply_gravity(self, ticks):
        ...
        grav_vectors = self.positions - self.positions[:, None, :]
        ...
        force_vectors = grav_vectors_normalized * inverse_square_dist * self.masses[None, :, None]
        ...
        force_vectors *= GRAVITY_CONSTANT * ticks
